...
__version__ = get_distribution('warp_rna').version


class _WRNA(torch.autograd.Function):
    @staticmethod
    def forward(ctx, log_probs, labels, frames_lengths, labels_lengths, blank=0):
        costs, ctx.grads = warp_rna_core.rna_loss(
...
def rna_loss(
    ...
    costs = _WRNA.apply(log_probs, labels, frames_lengths, labels_lengths, blank)
